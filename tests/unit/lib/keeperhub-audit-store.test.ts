...
describe("SLICE-126-10: keeperhub-audit-store", () => {
  ...
  it("SLICE-126-17: cap eviction removes oldest, not newest", () => {
    for (let i = 0; i < 500; i++) {
      auditStore.add({ source: `s${i}`, siteUrl: `https://${i}.com`, status: "recorded" });
    }
    expect(auditStore.size()).toBe(500);
    auditStore.add({ source: "new", siteUrl: "https://new.com", status: "recorded" });
    expect(auditStore.size()).toBe(500);
    const list = auditStore.list({ limit: 500 });
  ...
  it("SLICE-126-17: dedupe replace updates position to front", () => {
    auditStore.add({ source: "a", siteUrl: "https://a.com", status: "recorded", executionId: "e1" });
    auditStore.add({ source: "b", siteUrl: "https://b.com", status: "recorded" });
    auditStore.add({ source: "c", siteUrl: "https://c.com", status: "recorded" });
    expect(auditStore.list()[2].source).toBe("a");
    auditStore.add({ source: "a", siteUrl: "https://a.com", status: "failed", executionId: "e1" });
    const list = auditStore.list();
  ...
  it("SLICE-126-17: emitter on/off pairing under rapid add", () => {
    let count = 0;
    const handler = () => { count++; };
    ...
    auditStore.add({ source: "after", siteUrl: "https://after.com", status: "recorded" });
