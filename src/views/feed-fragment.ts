...
export function PassportCard({ nft }: { nft: NftInfo }) {
  ...
  const hashScanUrl = `https://hashscan.io/${network}/token/${tokenId}/${serial}`;
...
export function FeedFragment({ nfts }: { nfts: NftInfo[] }) {
  ...
  const hidden = nfts.slice(PAGE_SIZE);

  return html`<div class="space-y-3"
    >${visible.map((nft) => PassportCard({ nft }))}${hidden.map(
    (nft) =>
      html`<div class="hidden" data-paginated="true">${PassportCard({ nft })}</div>`,
  )}</div
  >${hidden.length > 0
      ? html`<button
        type="button"
