// EPIC-140 (SLICE-140-19): keeperhub hackathon page — assembled from sections.
import { html, raw } from "hono/html";
import { demoSection, auditSectionAnchor, sectionWrapper, type HackathonDemoConfig } from "./shared";
import { Hero, ArchitectureDiagram } from "./keeperhub-hero";
...
const KEEPERHUB_CFG: HackathonDemoConfig = {
  executingLabel: "KeeperHub workflow executing… recording on TrustRegistry (may take up to 1 min)",
  notEnabledMsg: "KeeperHub integration not enabled on this deployment",
  provisioningMsg: "Workflow or provisioning issue — check KEEPERHUB_API_KEY / workflow IDs",
  auditUnavailableMsg: "KeeperHub integration not enabled — audit trail unavailable",
};
...
export function KeeperHubHackathonPage(opts?: KeeperHubPageOpts) {
  return html`${Hero()}
    ${ArchitectureDiagram()}
    ${HowItWorks()}
    ${demoSection(KEEPERHUB_CFG)}
    ${auditSectionAnchor(KEEPERHUB_CFG)}
    ${StackSection()}
    ${ApiSection(opts)}
    ${FooterCta()}`;
}
