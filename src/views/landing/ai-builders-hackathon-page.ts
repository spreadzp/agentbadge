...
/**
 * AIBuildersHackathonPage — landing page for the AI Builders Hackathon submission.
 *
 * Cloned from KeeperHubHackathonPage with KeeperHub hackathon branding removed.
 * All technical content (scan, onchain, x402, MCP, audit trail) is preserved.
 */
    ...
    <script>
      (function() {
        ...
        function describeError(status, data) {
          if (status === 503) return "Onchain integration not enabled on this deployment";
          ...
          if (status === 502) return "Workflow or provisioning issue — check API keys / workflow IDs";
      ...
      (function () {
        ...
        fetch("/api/keeperhub/audit?limit=20")
          ...
          .catch(function(status) {
            if (status === 503) {
              showError("Onchain integration not enabled — audit trail unavailable");
            } else {
