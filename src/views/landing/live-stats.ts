...
/**
 * LiveStatsSection — landing page live stats with SSR + HTMX polling.
 * (SLICE-19-5)
 *
 * 4 cards: Passports Issued, Active Agents, Marketplace Tasks, Tier Upgrades.
 * SSR initial values from DashboardSsrData, HTMX polls /ui/stats every 10s.
 */
export function LiveStatsSection(data: {
  totalIssued: number;
  activeCount: number;
  ...
  return html`
    <section id="live-stats" class="px-4 py-16 md:px-8">
      <div class="mx-auto max-w-5xl">
        <div class="fade-in-up mb-8 text-center">
          <h2 class="text-2xl font-bold text-white md:text-3xl">Live Network Stats</h2>
          <p class="mt-2 text-sm text-slate-400">Real-time data from Hedera Network · Auto-refreshes every 10s</p>
        </div>
