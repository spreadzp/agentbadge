  ...
  >
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ${imageUrl
      ? html`<img src="${imageUrl}" alt="${agent.name}" loading="lazy" class="h-10 w-10 rounded-full object-cover flex-shrink-0" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'" />`
      : null
    }
      ...
      <div class="flex items-center gap-2">
        <div class="flex items-center gap-1">
          <a
            ...
            class="inline-flex min-h-6 min-w-6 items-center justify-center p-1 text-slate-400 hover:text-sky-400 transition-colors"
          ...
          <a
            ...
            class="inline-flex min-h-6 min-w-6 items-center justify-center p-1 text-slate-400 hover:text-amber-400 transition-colors"
    ...
    ${agent.skills && agent.skills.length > 0
      ? html`<div class="mt-2">
            <span class="text-xs text-slate-400 mr-1">Skills:</span>
...
export function AgentsFragment({ agents }: { agents: AgentWithActive[] }) {
  ...
  const remaining = agents.length - PAGE_SIZE;

  return html`<div class="space-y-3"
    >${visible.map((agent) => AgentRow({ agent }))}</div
  >${remaining > 0
      ? html`<button
        type="button"
        hx-get="/ui/agents/fragment?offset=${PAGE_SIZE}"
        hx-target="this"
        hx-swap="outerHTML"
