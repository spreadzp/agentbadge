...
export const categoryEnum = z
  .enum(["discovery", "documentation", "actionability", "machine_readable", "verification"])
  .describe("Rule category: discovery, documentation, actionability, machine_readable, verification");
...
export const checkTypeEnum = z
  .enum(["http_fetch", "schema_validation", "exact_match", "cross_evidence"])
  .describe("Check type: http_fetch (HTTP GET + parse), schema_validation (validate against JSON Schema), exact_match (string/structural equality), cross_evidence (compare two fetched sources on shared key)");
