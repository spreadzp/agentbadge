# Agent Readiness Specification v0.4 (Ruleset 2.2.0)

> **Spec version**: `0.4.0`
> **Ruleset reference**: `agent-readiness@2.2.0`
> **Status**: ACTIVE
> **Supersedes**: `AGENT-READINESS-SPEC-v0.3.md` (Ruleset 2.1.0, LOCKED — preserved for history)
> **Epic**: [EPIC-95: Semantic Checks](../../95-semantic-checks/EPIC-95-semantic-checks.md)
...
|---|---|---|
| 7 new categories | §A.2 | `pricing`, `rate_limits`, `error_semantics`, `retry_semantics`, `sandbox`, `versioning`, `agent_policy` — total 25 categories |
| `critical` severity | §A.3, §6.5 | New severity tier: "agents physically cannot proceed" (auth absent). Triggers total cap ≤ 30 regardless of category (§6.3 extension). Config-driven cap value. |
| §6.3 floor extension | §6.3 | Critical MISSING/CONFLICT → total ≤ 30 (harder than high-severity category floor ≤ 40) |
| `semantic_validation` check type | §A.4 | New check type: pure functions parsing structured sources (OpenAPI JSON, guide JSON, llms.txt) for semantic criteria. No LLM — deterministic boundary stated. |
| Semantic taxonomy | §14 (new) | Appendix B: 15-rule semantic taxonomy (AB-146..AB-160) with categories, severities, claims, three-valued status semantics |
| 6 rules retired | §14 | AB-010, AB-011, AB-012, AB-043, AB-064, AB-113 superseded by semantic upgrades (see gap analysis) |
| Per-rule claims | §1, §12 | Claims now ship per-rule (EPIC-94 D7 promise fulfilled): each semantic rule carries a real semantic claim string |
...
**v0.4 extension — critical floor:** If any `severity: critical` rule
resolves to GAP or CONFLICT, the total score is capped at ≤ 30
(regardless of category). This is harder than the high-severity category
floor (≤ 40) because critical-severity failures mean agents physically
cannot proceed (e.g. auth absent). The critical cap value is
configurable in the ruleset config (`criticalFloorCap`), not hardcoded.
...
|---|---|---|
| `critical` | Agents physically cannot proceed (e.g. auth absent) | Triggers critical floor: total ≤ 30 regardless of category (§6.3); triggers alert on regression |
...
The four-pillar model is an **aggregation layer** over the 25
categories (18 in v0.3 + 7 new in v0.4). Rules keep their `category`;
the pillar is derived via
...
to 18 categories (v0.3) to 25 categories (v0.4). The 7 new categories
in v0.4 support the semantic checks layer (EPIC-95).

| Value | Weight (v0.2) | Floor trigger | Pillar | Since |
|---|---|---|---|---|
| `discovery` | 15% | any `high` GAP/CONFLICT → total ≤ 40 | discovery | v0.1 |
| `documentation` | 15% | any `high` GAP/CONFLICT → total ≤ 40 | understandability | v0.1 |
| `actionability` | 10% | no floor | understandability | v0.1 |
| `machine_readable` | 10% | no floor | discovery | v0.1 |
| `verification` | 5% | no floor | verifiability | v0.1 |
| `content_negotiation` | 5% | no floor | discovery | v0.1 |
| `payments` | 10% | no floor | executability | v0.1 |
| `bazaar` | 5% | no floor | executability | v0.1 |
| `openapi` | 10% | no floor | discovery | v0.1 |
| `skills` | 5% | no floor | discovery | v0.1 |
| `agents_txt` | 3% | no floor | discovery | v0.1 |
| `webmcp` | 3% | no floor | discovery | v0.1 |
| `identity` | 2% | no floor | executability | v0.1 |
| `bot_auth` | 1% | no floor | executability | v0.1 |
| `infrastructure` | 1% | no floor | verifiability | v0.1 |
| `seo_aeo` | 5% | no floor | discovery | v0.1 |
| `accessibility` | 4% | no floor | understandability | v0.1 |
| `active_probing` | 5% | no floor | verifiability | v0.1 |
| `pricing` | 3% | any `high` GAP/CONFLICT → total ≤ 40 | understandability | v0.4 |
| `rate_limits` | 3% | any `high` GAP/CONFLICT → total ≤ 40 | understandability | v0.4 |
| `error_semantics` | 3% | any `high` GAP/CONFLICT → total ≤ 40 | executability | v0.4 |
| `retry_semantics` | 2% | no floor | executability | v0.4 |
| `sandbox` | 1% | no floor | executability | v0.4 |
| `versioning` | 2% | no floor | executability | v0.4 |
| `agent_policy` | 2% | no floor | verifiability | v0.4 |
...
| Value | Effect |
|---|---|
| `critical` | Triggers critical floor: total ≤ 30 regardless of category (§6.3); triggers alert on regression. Used sparingly — AB-153 only in v1 of this layer. |
...
**Note:** `critical` is introduced in v0.4 (EPIC-95). It represents
conditions where agents physically cannot proceed (e.g. auth absent).
The critical cap value is configurable via `criticalFloorCap` in the
ruleset config.
...
| `semantic_validation` | Pure function parsing a structured source (OpenAPI JSON, guide JSON, llms.txt) and checking semantic criteria: description presence, parameter constraints, example existence, error schema completeness, retry guidance, version policy. No LLM — deterministic boundary. (v0.4) |
| `cross_evidence` | Compares two already-fetched, already-parsed evidence sources on a shared key. Produces VERIFIED/CONFLICT/GAP. See §5. |
...
**Totals:** discovery = 8 categories, understandability = 5 categories,
executability = 8 categories, verifiability = 4 categories. 25/25 mapped.
...
25/25 coverage and zero orphans.
...
## Appendix B — Semantic Taxonomy (v0.4, EPIC-95)

> **Status:** SPECIFIED in SLICE-95-1. Implementation in SLICE-95-2..95-4.
> **Gap analysis:** See `docs/EPICS/95-semantic-checks/semantic-gap-analysis.md`

### B.1 Taxonomy — 15 semantic rules

| Rule ID | Category | Severity | Pillar | Check type | Claim |
|---|---|---|---|---|---|
| AB-146 | actionability | high | understandability | semantic_validation | Every OpenAPI operation has a meaningful description |
| AB-147 | documentation | medium | understandability | semantic_validation | Parameters have descriptions + constraints (min/max/enum) |
| AB-148 | documentation | medium | understandability | semantic_validation | Request/response examples exist in the spec |
| AB-149 | error_semantics | high | executability | semantic_validation | 4xx responses declared with schemas + descriptions |
| AB-150 | pricing | high | understandability | semantic_validation | Machine-readable pricing (guide/llms/openapi ext/well-known) |
| AB-151 | rate_limits | high | understandability | semantic_validation | Machine-readable limits + over-limit behavior |
| AB-152 | verification | medium | verifiability | cross_evidence | Declared values consistent across sources (cross-evidence) |
| AB-153 | bot_auth | critical | executability | semantic_validation | Auth scheme + credential location + how to obtain |
| AB-154 | retry_semantics | medium | executability | semantic_validation | Idempotency + Retry-After / retry guidance declared |
| AB-155 | versioning | medium | executability | semantic_validation | Version declared + deprecation/sunset policy |
| AB-156 | sandbox | low | executability | semantic_validation | Test environment advertised |
| AB-157 | agent_policy | medium | verifiability | semantic_validation | Machine-readable usage policy for automated clients |
| AB-158 | discovery | medium | discovery | semantic_validation | Guide declares capabilities/endpoints list |
| AB-159 | actionability | medium | understandability | semantic_validation | Per-capability constraints documented (refund window etc.) |
| AB-160 | documentation | low | understandability | semantic_validation | Machine-readable support contact/escalation |

### B.2 Retired rules (superseded by semantic upgrades)

| Old rule | Old category | Superseded by | Reason |
|---|---|---|---|
| AB-010 | machine_readable | AB-150 | Narrow content_parse → broad semantic_validation across sources |
| AB-011 | machine_readable | AB-151 | Narrow content_parse → broad semantic_validation with over-limit behavior |
| AB-012 | machine_readable | AB-149 | Narrow content_parse → broad semantic_validation with 4xx schemas + descriptions |
| AB-043 | openapi | AB-155 | Narrow content_parse → broad semantic_validation with deprecation/sunset |
| AB-064 | documentation | AB-153 | Narrow content_parse → broad semantic_validation with auth scheme + credential location |
| AB-113 | discovery | AB-157 | Narrow http_fetch → broad semantic_validation for automated client policy |

### B.3 Complementary rules (kept, not retired)

| Rule | Reason kept |
|---|---|
| AB-047 | Runtime header check — complementary to AB-151 (which checks declared limits) |
| AB-128 | OAuth2 preference — complementary to AB-153 (which checks auth scheme clarity) |
| AB-009 | Capability inference coverage — complementary to AB-158 (which checks guide declares capabilities) |

### B.4 Three-valued status semantics for semantic_validation

Each semantic_validation rule produces one of three statuses:

| Status | Condition |
|---|---|
| `VERIFIED` | All required semantic fields present and non-trivial (e.g. description > 10 chars, example exists) |
| `GAP` | Required field absent or trivially empty (e.g. description = "" or "TODO") |
| `NOT_APPLICABLE` | Rule's applicability predicate excludes this scope (e.g. sandbox rule on an API with no test env) |

`CONFLICT` is not produced by semantic_validation rules — they inspect a
single source. Cross-source consistency is handled by AB-152
(`cross_evidence` check type).

### B.5 Deterministic boundary

`semantic_validation` checks are pure functions over parsed structured
documents (OpenAPI JSON, guide JSON, llms.txt). No LLM calls. The
deterministic boundary from §2 is preserved: the fact of
VERIFIED/GAP/NOT_APPLICABLE is always produced by deterministic code.
